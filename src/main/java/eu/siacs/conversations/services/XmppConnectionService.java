...
public class XmppConnectionService extends Service implements OnPhoneContactsLoadedListener {
    ...
    public void attachFileToConversation(final Conversation conversation,
        ...
        if (path != null) {
            ...
            Log.i(TAG, "message.getEncryption()=" + message.getEncryption());
    ...
    public void sendMessage(final Message message) {
        ...
        if (account.getStatus() == Account.State.ONLINE
            ...
            Log.i(TAG, "1 message.getType()=" + message.getType());
    ...
    public void publishAvatar(final Account account,
        ...
        publishAvatar0153(account, image, callback);
        if (true) {
            return;
        }
    ...
    public void publishAvatar0153(final Account account,
                                  final Uri image,
                                  final UiCallback<Avatar> callback) {
        Bitmap.CompressFormat format = Config.AVATAR_FORMAT;
        // format = Bitmap.CompressFormat.PNG;
        int size = Config.AVATAR_SIZE;
        final Avatar avatar = getFileBackend()
                .getPepAvatar(image, size, format);
        if (avatar != null) {
            avatar.height = size;
            ...
            final IqPacket packet = this.mIqGenerator.setAvatar0153(avatar);
            packet.setFrom(account.getJid().toBareJid());
            Log.i(TAG, "0153=" + packet);
            this.sendIqPacket(account, packet, new OnIqPacketReceived() {
                @Override
                public void onIqPacketReceived(Account account, IqPacket result) {
                    if (result.getType() == IqPacket.TYPE.RESULT) {
                        avatar.sha1sum = result.getAttribute("id");
                        Log.d(TAG, "0153=" + result);
                        callback.success(avatar);
                        PresencePacket presencePacket = new PresencePacket();
                        presencePacket.setFrom(account.getJid());
                        presencePacket.addChild("x", "vcard-temp:x:update").addChild("photo").setContent(avatar.sha1sum);
    ...
    /**
     * Implementation of <a href="http://www.xmpp.org/extensions/xep-0153.html">XEP-0153</a>
     *
     * @param account
     * @param fromJid
     * @param sha1sum
    ...
    public void fetchAvatar(Account account, Jid fromJid, String sha1sum) {
        ...
        if (contact != null) {
            ...
            avatar.type = "image/png";
            Log.i(TAG, "isAvatarCached=" + getFileBackend().isAvatarCached(avatar) + " avatar.getFilename()=" + avatar.getFilename());
            ...
            iqPacket.setTo(fromJid.toBareJid());
            iqPacket.setFrom(account.getJid());
            ...
            sendIqPacket(account, iqPacket, new OnIqPacketReceived() {
                ...
                public void onIqPacketReceived(Account account, IqPacket packet) {
                    ...
                    if (vCard != null) {
                        ...
                        if (photo != null) {
                            Element type = photo.findChild("TYPE");
                            Element binval = photo.findChild("BINVAL");
                            Log.d(TAG, "binval.getContent().length=" + binval.getContent().length());
                            if (binval != null && type != null) {
                                avatar.type = type.getContent();
                                avatar.owner = contact.getJid();
                                avatar.image = binval.getContent();
                                boolean saved = getFileBackend().save(avatar);
                                Log.d(TAG, "saved=" + saved + " avatar.getFilename()=" + avatar.getFilename() + " fileBackend.isAvatarCached(avatar)=" + fileBackend.isAvatarCached(avatar));
                                if (saved) {
                                    contact.setAvatar(avatar.getFilename());
                                    databaseBackend.updateAccount(account);
                                    getAvatarService().clear(contact);
